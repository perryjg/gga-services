  ...
  ADD COLUMN sent_gov_date DATETIME,
  ADD COLUMN min_pass_date DATETIME,
  ADD COLUMN max_pass_date DATETIME,
  ...
  UPDATE bills_attributes a, (
  SELECT ba.id,
    MIN(status_date) AS min_pass_date,
    MAX(status_date) AS max_pass_date,
          COUNT(*) AS chamber_passes
  FROM bill_status_listings bsl
  JOIN bills_attributes ba
  ON ba.id=bsl.`bill_id`
  ...
  SET a.min_pass_date=t.min_pass_date,
      a.max_pass_date=t.max_pass_date,
      a.chamber_passes=t.chamber_passes
  ...
  UPDATE bills_attributes a, (
  SELECT ba.id,
    MIN(status_date) AS out_committee_date
  FROM bill_status_listings bsl
  JOIN bills_attributes ba
  ON ba.id=bsl.`bill_id`
  WHERE CODE IN ('HCFR','SCFR')
  GROUP BY ba.id) t
  ...
  UPDATE bills_attributes a, (
  SELECT ba.id,
    MIN(status_date) AS sent_gov_date
  FROM bill_status_listings bsl
  JOIN bills_attributes ba
  ON ba.id=bsl.`bill_id`
  WHERE CODE IN ('HSG','SSG')
  GROUP BY ba.id) t

  SET a.sent_gov_date=t.sent_gov_date
  ...
  UPDATE bills_attributes a, (
  SELECT id,
      DATEDIFF(MAKEDATE(YEAR(min_pass_date),121),min_pass_date) AS days_from_may_passed_first
